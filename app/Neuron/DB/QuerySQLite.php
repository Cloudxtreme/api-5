...
class QuerySQLite
{
	...
	public static function insert ($table, array $set)
	{
		...
		foreach ($set as $k => $v)
		{
			...
			//TODO: Prepare binValues with SQLite3
			//$str_values .= $k . ' = ?, ';
		...
		$query = "$query ( $str_columns ) VALUES ( $str_values )";
	...
	*/
	...
	public function execute ()
	{
		...
		// SQL LOGGING :: BEGIN
		if (defined ('DB_CUSTOM_LOG_SQLITE') && DB_CUSTOM_LOG_SQLITE) {
			
			$sSQL = $query;
		
			$error_log_folder   = str_replace('\\','/', dirname(__FILE__) .'/../../../../logs');
			$error_log_filename = "$error_log_folder/sqlite_log.txt";
			$error_log_extended_filename = "$error_log_folder/sqlite_log_extended.txt";
				
			//$str_context = print_r($context, true);
				
			//$str_debug_backtrace = print_r(debug_backtrace(), true);
				
			$str_log = "-- ----------------------------------------------------------------------------\n$sSQL;\n";
				
			file_put_contents($error_log_filename, $str_log, FILE_APPEND | LOCK_EX);
		}
